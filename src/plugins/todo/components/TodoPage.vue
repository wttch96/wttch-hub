...
import {
  ...
  renameLane, todoState, toggleTodo, type TodoItem, type TodoPriority,
} from '../store';
...
const draft = reactive({ title: '', notes: '', dueAt: '', priority: props.defaultPriority as TodoPriority, laneId: firstActiveLane() });
...
const submit = () => {
  ...
  draft.title = ''; draft.notes = ''; draft.dueAt = ''; draft.priority = props.defaultPriority;
...
<template>
  <section class="todo-page">
    ...
    <form class="composer card" @submit.prevent="submit">
      ...
      <textarea v-model="draft.notes" maxlength="500" rows="2" placeholder="任务描述（可选）" aria-label="待办备注" />
...
.lane-cards { display: grid; gap: 7px; min-height: 8px; } .task-card { padding: 10px; border: 1px solid var(--hairline); border-radius: 9px; background: var(--panel); box-shadow: 0 2px 7px color-mix(in srgb, var(--text) 5%, transparent); cursor: grab; transition: opacity .15s, transform .15s; } .task-card:active { cursor: grabbing; } .task-card.dragging { opacity: .35; } .task-card.insert-before { box-shadow: 0 -3px 0 var(--accent), 0 2px 7px color-mix(in srgb, var(--text) 5%, transparent); } .task-top { gap: 5px; } .task-top .grip { flex: 0 0 auto; color: var(--text-secondary); } .task-top strong { min-width: 0; flex: 1; font-size: 11px; line-height: 1.35; } .task-top > button { display: inline-flex; padding: 2px; border: 0; background: transparent; color: var(--text-secondary); cursor: pointer; } .task-top > button:hover { color: var(--danger); } .priority { padding: 2px 5px; border-radius: 5px; background: var(--accent-weak); color: var(--text-secondary); font-size: 8px; } .priority.high { background: color-mix(in srgb, var(--danger) 14%, transparent); color: var(--danger); } .priority.low { color: var(--success); } .task-card p { margin: 7px 0; color: var(--text-secondary); font-size: 10px; line-height: 1.45; white-space: pre-wrap; } .task-meta { justify-content: space-between; gap: 7px; margin-top: 7px; } .task-meta time { display: flex; align-items: center; gap: 3px; color: var(--text-secondary); font-size: 9px; } .task-meta time.overdue { color: var(--danger); } .task-meta button { gap: 3px; margin-left: auto; border: 0; background: transparent; color: var(--text-secondary); font-size: 9px; cursor: pointer; } .task-meta button.done, .task-meta button:hover { color: var(--success); }
