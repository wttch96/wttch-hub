...
const formatRate = (value: number) => value < 1024 * 1024 ? `${(value / 1024).toFixed(1)} KB/s` : `${(value / 1024 / 1024).toFixed(1)} MB/s`;
const refresh = async () => {
...
<template>
  <section class="monitor">
    <header class="monitor-head"><div><p class="eyebrow">SYSTEM MONITOR</p><h2>设备状态</h2></div><Activity :size="18" /></header>
    <div class="metrics">
      <div class="metric"><Cpu :size="16" /><span>CPU</span><strong>{{ stats.cpu.toFixed(0) }}%</strong><div class="meter"><i :style="{ width: `${stats.cpu}%` }" /></div></div>
      <div class="metric"><Monitor :size="16" /><span>显卡</span><strong>{{ stats.gpu.toFixed(0) }}%</strong><div class="meter"><i :style="{ width: `${stats.gpu}%` }" /></div></div>
      <div class="metric"><MemoryStick :size="16" /><span>内存</span><strong>{{ stats.memory.toFixed(0) }}%</strong><div class="meter"><i :style="{ width: `${stats.memory}%` }" /></div></div>
      <div class="metric io"><HardDrive :size="16" /><span>磁盘 IO</span><strong>↓ {{ readRate }}</strong><small>↑ {{ writeRate }}</small></div>
      <div class="metric io"><Wifi :size="16" /><span>网络</span><strong>↓ {{ downloadRate }}</strong><small>↑ {{ uploadRate }}</small></div>
    </div>
  </section>
</template>
