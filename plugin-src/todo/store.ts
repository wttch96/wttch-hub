...
const validLanes = (value: unknown): TodoLane[] =>
  Array.isArray(value)
    ? value
        .filter((lane): lane is TodoLane =>
          Boolean(
            lane &&
            typeof lane === 'object' &&
            typeof (lane as TodoLane).id === 'string' &&
            typeof (lane as TodoLane).title === 'string',
          ),
        )
        .map((lane, index) => ({
          id: lane.id,
          title: lane.title.trim() || `泳道 ${index + 1}`,
          color: /^#[0-9a-f]{6}$/i.test(lane.color)
            ? lane.color
      ...
      return {
        ...
        body:
          typeof item.body === 'string'
            ? item.body
            : typeof (item as TodoItem & { notes?: string }).notes === 'string'
...
export const connectTodoStore = (nextApi?: PluginComponentApi) => {
  ...
  if (!subscription)
    subscription = api.storage.onDidChange((key, value) => {
      if (key === 'lanes') {
...
export const disconnectTodoStore = () => {
...
const laneFor = (id?: string) =>
  state.lanes.find(lane => lane.id === id) ??
  state.lanes.find(lane => !lane.completed) ??
  state.lanes[0];
const normalizeLaneOrder = (laneId: string) =>
  state.items
...
export const addLane = (title: string) => {
  ...
  state.lanes.splice(Math.max(0, state.lanes.length - 1), 0, {
    id: makeId(),
    title: normalized,
    color: laneColors[state.lanes.length % laneColors.length],
    completed: false,
    createdAt: new Date().toISOString(),
  });
  persistLanes();
...
export const removeLane = (id: string) => {
  ...
  const fallback =
    state.lanes.find(entry => entry.id !== id && !entry.completed) ??
    state.lanes.find(entry => entry.id !== id);
  if (!fallback) return false;
  state.items
    .filter(item => item.laneId === id)
    .forEach(item => {
      item.laneId = fallback.id;
      item.completed = fallback.completed;
      item.completedAt = fallback.completed ? new Date().toISOString() : undefined;
    });
...
// 可以添加 describe 描述，但是似乎 ai 更懂属性名？
export const AddTodoArgSchema = z.object({
  title: z.string(),
...
type AddTodoArg = z.infer<typeof AddTodoArgSchema>;

export const addTodo = (arg: AddTodoArg) => {
  const title = arg.title.trim();
  const lane = laneFor(arg.laneId);
  ...
  state.items.push({
    ...
    title,
    body: arg.body?.trim() ?? '',
    dueAt: arg.dueAt || undefined,
    priority: arg.priority ?? 'normal',
    completed: lane.completed,
    laneId: lane.id,
    order,
    createdAt: new Date().toISOString(),
    completedAt: lane.completed ? new Date().toISOString() : undefined,
  });
  persistItems();
  return true;
};
...
export const moveTodo = (id: string, targetLaneId: string, beforeId?: string) => {
  ...
  const targetItems = state.items
    .filter(entry => entry.laneId === lane.id && entry.id !== id)
    .sort((a, b) => a.order - b.order);
...
export const RemoveTodoArgSchema = z.object({
  id: z.string(),
});

export type RemoveTodoArg = z.infer<typeof RemoveTodoArgSchema>;
