...
export const createThemeExtension = (storage: PluginStorageApi, settings: PluginSettingsApi): ThemeExtensionController => {
  ...
  return {
    registerColor(key, defaultValue, description) {
      if (!validKey(key)) throw new Error('主题颜色 key 必须是 2–64 位的小写字母、数字、点或连字符。');
      ...
      emit();
      return { dispose: () => { if (colors.delete(key)) emit(); } };
    },
    getColor: (key) => {
      const color = colors.get(key);
      return color ? (color.settingKey ? settings.get<string>(color.settingKey, color.defaultValue) : values()[key] ?? color.defaultValue) : undefined;
    },
    setColor(key, value) {
    ...
    getColors: () => [...colors.values()].map(color => ({ key: color.key, defaultValue: color.defaultValue, description: color.description, value: color.settingKey ? settings.get<string>(color.settingKey, color.defaultValue)! : values()[color.key] ?? color.defaultValue })),
    ...
    registerThemeColor(key, settingKey, description) {
      ...
      if (!defaultValue || !validColor(defaultValue)) throw new Error(`主题设置 ${settingKey} 不是有效颜色。`);
      if (colors.has(key)) throw new Error(`主题颜色 ${key} 已注册。`);
