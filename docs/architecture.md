...
普通插件设置、插件私有 storage、导航偏好和主页 Widget 布局保存在渲染进程本地存储。主进程的用户数据目录由 Electron `app.getPath('userData')` 决定；其中包含 AI 配置、微信配置、桌面行为偏好、托管插件 ZIP 与恢复暂存文件。实际路径因系统、应用 ID 和安装方式不同，应通过运行时 Electron API 获取，不应在代码中写死。
